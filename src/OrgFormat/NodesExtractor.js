...
export const getNodesLineRanges = lines =>
  R.pipe(
    addLineNumbers,
    getHeadlinesLineNumbers,
    R.converge(R.zip, [R.identity, R.pipe(R.drop(1), R.append(lines.length))])
  )(lines);

const mapRangesToContent = (lines, ranges) =>
  ranges.map((range, position) =>
    R.merge(preParseHeadline(lines[range[0]]), {
      position: position + 1,
...
export const extractNodesFromLines = R.converge(mapRangesToContent, [
  R.identity,
  getNodesLineRanges,
]);
