...
const useRestAsContentAndRemoveInput = input =>
  R.append({ content: input[1].join('\n') }, input[0]);

export const parseRawNode = R.converge((...args) => R.mergeAll(args), [
  R.pipe(R.prop('rawHeadline'), parseHeadline),

  R.pick(['level', 'rawContent', 'position']),
]);
...
const mapMetadataLinesToObject = R.pipe(
  R.map(R.pipe(R.match(fileMetadataR), R.drop(1))),
  R.fromPairs
);
...
export const parseFileContent = R.pipe(
  ...
  R.applySpec({
    metadata: R.pipe(R.head, mapMetadataLinesToObject),
    description: R.pipe(R.last, R.join('\n')),
  })
);
...
export const parseNodes = R.pipe(preParseNodes, R.map(finishParsing));
