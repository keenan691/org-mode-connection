...
export const createCreatorsFromRegex = regexes => {
  ...
  const createLineObjects = R.mapObjIndexed((regexObj, key) =>
    R.pipe(
  ...
  const mapObjToCreators = R.map(obj => (content, additionalProps) =>
    R.merge(obj, { content, ...additionalProps })
  );

  return R.pipe(createLineObjects, mapObjToCreators)(regexes);
};

const beginsBySpecialBlock = R.pipe(R.head, R.equals(0));
const propOf = R.flip(R.prop);

const safeAddFirstIndex = R.unless(R.pipe(R.head, R.equals(0)), R.prepend(0));

const createParsers = (createParserFunction, parsersDescription) =>
  R.pipe(R.toPairs, R.map(([name, val]) => createParserFunction(name, val)))(
    parsersDescription
  );
...
const isTextFaceStart = R.curry(
  (line, idx) =>
    spaceChars.includes(line[idx - 1]) && !spaceChars.includes(line[idx + 1])
);
...
const createOrgTextFaceParser = (type, specialChar) => ([objects, line]) => {
  const findSpecialCharsIndexes = line =>
    indexesOf(line, new RegExp('\\' + specialChar, 'g'));

  const getCharType = R.cond([
    [isTextFaceStart(line), R.always('s')], // is section start
    [
      isTextFaceEnd(line),
      R.always('e'), // is section end
    ],
  ]);
...
const parseLine = line => {
  ...
  return R.pipe(...textFacesParsers, ...linksParsers, regularTextParser)(
    innerRepr
  );
};
