...
const importFile = (filepath, type = 'agenda') => {
  ...
  const addToDb = ([fileStat, parsedObj]) =>
    connectDb().then(realm =>
      realm.write(() => {
  ...
  return Promise.all([getFileStats, getFileContent])
    .then(transform)
    .then(addToDb);
};
...
export const getNewExternalMtime = file =>
  FileAccess.stat(file.path).then(stat => {
...
const getNodesFromDbAsArray = file =>
  Queries.getNodes('file = $0', file).then(nodes => realmResultToArray(nodes));
...
const partitionEachGroupByIdPossession = R.map(
  R.partition(node => node.id === undefined)
);

const groupByChangedAndNotChanged = R.groupBy(nodesGroup => {
...
const spreadNotSymmetricalGroups = groupedNodes => {
  ...
  if (notSymetricalGroups) {
    notSymetricalGroups.forEach(group => {
      ...
      for (
        let i = 0;
        i < Math.max(newNodes.length, existingNodes.length);
        i++
      ) {
...
const rejectNotChangedNodes = changes => {
  ...
  const eqPosition = (savedObj, newProps) =>
    fileNodes[newProps.position - 1].id === savedObj.id;
  const eqTags = (savedObj, newProps) =>
    R.equals(
      Array.from(savedObj.tags).map(R.prop('name')),
      newProps.tags.map(R.prop('name'))
    );
  ...
  return R.over(
    R.lensPath(['externalChanges', 'changedNodes']),
    R.pipe(
      R.reject(([savedObj, newProps]) =>
        R.allPass([
          R.eqProps('todo'),
...
const addNewNodePositions = changes => {
  ...
  R.pipe(toBatch, R.map(updatePosition))(changes.externalChanges.addedNodes);
...
export const getChanges = async file => {
  ...
  results.externalChanges = R.pipe(
    ...
    R.evolve({
      addedNodes: R.pipe(R.flatten, R.sortBy(R.prop('position'))),
      deletedNodes: R.flatten,
      changedNodes: R.pipe(R.map(R.transpose), R.unnest, R.map(R.reverse)),
    })
  )(data);
...
const applyLocalChanges = async changes => {
  const nodes = Array.from(changes.file.nodes.sorted('position'))
    .map(n => Export(n))
    .join('\n');
...
const applyExternalChanges = type => async changes => {
  ...
  if (type === 'delete' && externalChanges.deletedNodes) {
    ...
    await Queries.addNodes(
      preparedNodes,
      { fileId: changes.file.id },
      true,
      false
    );
  } else if (type === 'update' && externalChanges.changedNodes) {
...
const onlyExternalChangesP = changes =>
  !changes.localChanges && changes.externalChanges;
// const bothExternalAndLocalChangesP = changes => changes.localChanges && changes.externalChanges;
const bothExternalAndLocalChangesP = changes =>
  changes.file.isChanged && changes.externalChanges;

const generateReportAndUpdateStatus = changes => {
  if (changes.externalChanges === null && changes.localChanges === null)
    return null;

  const changesToSummary = {
    file: R.prop('path'),
    localChanges: R.unless(R.isNil, R.length),
    externalChanges: R.unless(
      R.isNil,
      R.pipe(
        R.evolve({
          addedNodes: R.length,
  ...
  if (changes.localChanges !== null) updateNodesAsSynced(changes.localChanges);
...
const fixEmptyExternalChanges = R.when(
  R.propEq('externalChanges', { changedNodes: [] }),
  R.assoc('externalChanges', null)
);
...
const syncAllFiles = () =>
  Queries.getFiles()
    ...
    .then(files =>
      Promise.all(
        files.filter(file => file.path != null).map(file => syncFile(file))
      )
    )
